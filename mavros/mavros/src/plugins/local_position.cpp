...
private:
	...
	void handle_local_position_ned(const mavlink::mavlink_message_t *msg, mavlink::common::msg::LOCAL_POSITION_NED &pos_ned)
	{
		...
		// auto enu_position = ftf::transform_frame_ned_enu(Eigen::Vector3d(pos_ned.x, pos_ned.y, pos_ned.z));
		// auto enu_velocity = ftf::transform_frame_ned_enu(Eigen::Vector3d(pos_ned.vx, pos_ned.vy, pos_ned.vz));
		Eigen::Vector3d ned_position(pos_ned.x, pos_ned.y, pos_ned.z);
		Eigen::Vector3d ned_velocity(pos_ned.vx, pos_ned.vy, pos_ned.vz);
		//--------------- Get Odom Information ---------------//
		// Note this orientation describes baselink->ENU transform
		auto ned_orientation_msg = m_uas->get_attitude_orientation_ned();
		// auto baselink_angular_msg = m_uas->get_attitude_angular_velocity_enu();
		auto ned_imu_msg = m_uas->get_attitude_imu_ned();

		Eigen::Quaterniond ned_orientation;
		tf::quaternionMsgToEigen(ned_orientation_msg, ned_orientation);
		// auto baselink_linear = ftf::transform_frame_enu_baselink(ned_velocity, ned_orientation.inverse());
		Eigen::Matrix3d _R = ned_orientation.toRotationMatrix();
		// Eigen::Vector3d ned_acc;
		Eigen::Vector3d ned_acc(ned_imu_msg->linear_acceleration.x,
								ned_imu_msg->linear_acceleration.y,
								ned_imu_msg->linear_acceleration.z);
		ned_acc = _R*ned_acc; ned_acc(2) += 9.8;
		auto odom = boost::make_shared<nav_msgs::Odometry>();
		odom->header = m_uas->synchronized_header(frame_id, pos_ned.time_boot_ms);
		odom->child_frame_id = tf_child_frame_id;

		tf::pointEigenToMsg(ned_position, odom->pose.pose.position); // Position
		odom->pose.pose.orientation = ned_orientation_msg;			 // Orientation
		tf::vectorEigenToMsg(ned_velocity, odom->twist.twist.linear);// Velocity
		tf::vectorEigenToMsg(ned_acc, odom->twist.twist.angular);	 // Acc
