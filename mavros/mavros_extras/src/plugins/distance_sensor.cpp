...
private:
	...
	void handle_distance_sensor(const mavlink::mavlink_message_t *msg, mavlink::common::msg::DISTANCE_SENSOR &dist_sen)
	{
		...
		auto range = boost::make_shared<sensor_msgs::FluidPressure>();
		...
		range->fluid_pressure = dist_sen.current_distance * 1E-2;	// in meters
...
DistanceSensorItem::Ptr DistanceSensorItem::create_item(DistanceSensorPlugin *owner, std::string topic_name)
{
	...
	if (!p->is_subscriber)
		p->pub = owner->dist_nh.advertise<sensor_msgs::FluidPressure>(topic_name.c_str(), 10);//"/lidar_ns/lidar_filtered"
	else
