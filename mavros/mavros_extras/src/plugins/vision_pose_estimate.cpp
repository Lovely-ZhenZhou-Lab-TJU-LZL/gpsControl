...
public:
	...
	void initialize(UAS &uas_)
	{
		...
		if (tf_listen) {
			...
			vision_sub = sp_nh.subscribe("/vio_data_rigid1/mav_lf", 10, &VisionPoseEstimatePlugin::vision_cb, this);//接收ukf输出的位姿信息，NED
...
private:
	...
	void send_vision_estimate_inputNED(const ros::Time &stamp, const Eigen::Affine3d &tr)
	{
		/**
		 * @warning Issue #60.
		 * This now affects pose callbacks too.
		...
		auto rpy = ftf::quaternion_to_rpy( Eigen::Quaterniond(tr.rotation()) );
	...
	void vision_cb(const geometry_msgs::PoseStamped::ConstPtr &req)
	{
		...
		send_vision_estimate_inputNED(req->header.stamp, tr);
	}

	/*void vision2_cb(const nav_msgs::Odometry::ConstPtr &req)
	{
		geometry_msgs::Pose tmp_msg = req.pose.pose;
		geometry_msgs::Pose tmp_msg2;
		tmp_msg2.position.x = tmp_msg.position.x;
		tmp_msg2.position.y = tmp_msg.position.z;
		tmp_msg2.position.z = -tmp_msg.position.y;
		tmp_msg2.orientation.w = tmp_msg2.orientation.w;		
		tmp_msg2.orientation.x = tmp_msg2.orientation.x;		
		tmp_msg2.orientation.y = tmp_msg2.orientation.z;		
		tmp_msg2.orientation.z = -tmp_msg2.orientation.y;
		Eigen::Affine3d tr;
		tf::poseMsgToEigen(tmp_msg2, tr);
