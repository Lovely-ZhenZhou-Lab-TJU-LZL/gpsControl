...
void Controller::controller_loop() {
    ...
    ros::WallRate ctrl_rate(80);//100;
    ROS_INFO("controller run at 80Hz");
                                        ...
                                        ctrl_msg::SetTakeoffLand::Response& res) 
{
    ...
    if (req.takeoff) {
        ...
        while(ros::ok() && ros::Time::now()-start_land_task_time < ros::Duration(8.0)) {
            ...
            float temp_lidar_z = downward_lidar_data;
            pthread_mutex_unlock(&lidar_data_mutex);
            if (temp_lidar_z < 0.15f && temp_lidar_z!=0 
                && fabs(state_now.Vel(2)) < 1.0f) 
            {
...
void Controller::down_ward_lidar_cb(const sensor_msgs::FluidPressure::Ptr msg) {
    pthread_mutex_lock(&lidar_data_mutex);
    downward_lidar_data = msg->fluid_pressure;
    pthread_mutex_unlock(&lidar_data_mutex);
}
