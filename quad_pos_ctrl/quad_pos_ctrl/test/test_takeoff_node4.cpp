...
int main(int argc, char **argv) {
    ...
    ros::ServiceClient takeoff_land_srv = node.serviceClient<ctrl_msg::SetTakeoffLand>(uav_srv_name);
    ros::Publisher ctrl_ref_pub = node.advertise<ctrl_msg::ctrl_ref>(uav_ref_topic_name,10);

    ctrl_msg::SetTakeoffLand set_takeoff_land;
    set_takeoff_land.request.takeoff = true;
    set_takeoff_land.request.takeoff_altitude = 1.8f;
